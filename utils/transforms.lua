...
function M.Scale(size, interpolation)
    ...
    return function(input)
        local sideLen = math.min(input:size(3), input:size(2))

        local ratio = {
            h = input:size(2)/sideLen,
            w = input:size(3)/sideLen
        }
        return image.scale(input, size*ratio.w, size*ratio.h, interpolation)
    end
end
...
function M.RandomCrop(size, padding)
    ...
    return function(input)
        if padding > 0 then
            local temp = input.new(3,
                input:size(2) + 2*padding, input:size(3) + 2*padding)
            temp:zero()
...
function M.Rotation(deg)
    return function(input)
        if deg ~= 0 then
            input = image.rotate(input,
                (torch.uniform() - 0.5) * deg * math.pi / 180, 'bilinear')
        end
        return input
    end
end

function M.Affine(deg)
    return function(input)
        local theta = math.pi / 180 * deg
        local theta_smp = torch.uniform(-theta, theta)
        local tan = torch.tan(theta_smp)
        local diag = {tan, 0}
        local order = torch.randperm(2)
        local affineMat = torch.FloatTensor(
            {{1, diag[order[1]]}, {diag[order[2]], 1}}
            )
        input = image.affinetransform(input:double(), affineMat)
        return input
    end
end

function M.BrushEdge()
    return function(input)
        -- make edge map into brushed edge map
        local dice = torch.uniform(0,1)
        local th
        if dice < 0.5 then
            th = torch.uniform(0.5,0.7)
        else
            th = torch.uniform(0.8,0.95)
        end

        local lt = torch.lt(input, th):typeAs(input)
        local ge = torch.ge(input, th):typeAs(input)
        local rnd = lt.new():resizeAs(lt):uniform(0,0.3)
        input:cmul(ge):addcmul(lt, rnd)
...
function M.GammaCorrect(gamma, constant)
    constant = constant or 1
    return function(input)
        input:pow(gamma):mul(constant)
        return input
    end
end
...
function M.GammaJitter(gammas)
    assert(torch.type(gammas) == 'table', 'gammas should be a table')
    gammas = #gammas == 0 and {1} or gammas

    return function(input)
        local output = {}
        for _, gamma in ipairs(gammas) do
            local gammaCorrect = M.GammaCorrect(gamma, 1)
            outptu[#output+1] = gammaCorrect(input)
        end

        -- View as mini-batch
        for i, img in ipairs(output) do
            local size = img:size():totable()
            output[i] = img:view(1, unpack(size))
        end
...
function M.Occlusion(mask)
    assert(mask:dim() == 4, 'mask should be nxcxhxw tensor')
    local invmask = mask:eq(0):typeAs(mask)
    local noise = mask:clone():uniform():cmul(mask)
    --local noise = 1 - mask
    return function(input)
        local output = {input}
        local nMask = mask:size(1)
        for i = 1,nMask do
            local _out = torch.cmul(invmask[i]:expandAs(input), input)
            _out:add(noise[i]:expandAs(input))
            --_out:cmul(noise[i]:expandAs(input))
            table.insert(output, _out)
        end
