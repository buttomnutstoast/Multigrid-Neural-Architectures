...
function dataset:__init(...)
    ...
    -- get dataset
    local filePath = paths.concat(self.path, 'cifar100-'..self.protocol..'.t7')
    local file = torch.load(filePath)

    self.imageData = file.data:float():view(-1, 3072)
    self.imageClass = file.label:float()
    self.imageCoarseClass = file.labelCoarse:float()

    self.imageClass:add(1) -- make label range from 1 to 10
    self.imageCoarseClass:add(1)
